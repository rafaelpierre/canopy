...
import { fileColor, fileIcon } from './file-icons'
import { type GitignoreRule, parseGitignore } from './gitignore'
...
function isIgnored(entry: FlatEntry): boolean {
  ...
  for (let d = 1; d <= parts.length; d++) {
    ...
    if (ignored) {
      _ignoredCache.set(entry.path, true)
      return true
    }
  }
  _ignoredCache.set(entry.path, false)
...
async function loadTree(root: string) {
  ...
  const isFirstLoad = loadedRoot !== root
  loadedRoot = root
  try {
    tree = await invoke('read_dir_recursive', { path: root, depth: 0 })
    if (isFirstLoad) {
      // New project — start with everything collapsed and clean.
      expanded = new Set()
      childrenMap = new Map()
    } else {
      // Same project, fs change reload (e.g. uv add). Preserve user's expanded
      // state and refresh cached children for currently-open folders so new
      // files appear without requiring collapse+expand.
      const expandedPaths = Array.from(expanded)
      const refreshed = await Promise.all(
        expandedPaths.map(async (p) => {
          try {
            return [p, await invoke('read_dir_recursive', { path: p, depth: 0 })] as [
              string,
              FileEntry[],
            ]
          } catch {
      ...
      for (const r of refreshed) if (r) next.set(r[0], r[1])
      childrenMap = next
    }
  } catch (e) {
...
// Expand every ancestor folder of `filePath` (bounded by projectRoot) so the
// file is visible in the tree. Used on initial restore (file is already in a
// tab from last session) and on programmatic openFilePath changes (jump-to-
// definition). Idempotent — re-fetches nothing already cached.
async function expandToReveal(filePath: string) {
  if (!invoke || !projectRoot || !filePath.startsWith(projectRoot + '/')) return
  const rel = filePath.slice(projectRoot.length + 1)
  const segments = rel.split('/')
  if (segments.length <= 1) return // file is at root
  ...
  const toFetch = ancestors.filter((p) => !childrenMap.has(p))
  if (toFetch.length) {
...
onMount(async () => {
  ...
  cleanups.push(
    stores.openFilePath.subscribe((p: string | null) => {
      activeFile = p ?? ''
      if (p) {
        updateActiveFolders(p)
        expandToReveal(p)
      }
    }),
  )
  cleanups.push(
    stores.treeFontSize.subscribe((s: number) => {
      fontSize = s
    }),
  )
  cleanups.push(
    stores.projectRoot.subscribe(async (root: string | null) => {
      projectRoot = root
      if (root) {
        await loadTree(root)
        loadGitignore(root)
        invoke('watch_project', { root }).catch(() => {})
        // After tree loads, reveal the active file (may have been restored
        // before projectRoot was set, so the openFilePath subscriber's earlier
        // expandToReveal call no-op'd).
        if (activeFile) expandToReveal(activeFile)
      }
    }),
  )
  ...
  cleanups.push(
    await ipcMod.listen('dir:changed', () => {
      if (loadedRoot) {
        loadTree(loadedRoot)
        loadGitignore(loadedRoot)
      }
    }),
  )

  cleanups.push(
    stores.diagnosticFileSets.subscribe(
      ({
        errorPaths: e,
        warningPaths: w,
      }: {
        errorPaths: Set<string>
        warningPaths: Set<string>
      }) => {
        errorPaths = e
        warningPaths = w
      },
...
type FlatOrCreate =
  | FlatEntry
  | { _creating: true; dir: string; kind: 'file' | 'folder'; depth: number }
...
async function ctxDelete(entry: FlatEntry) {
  ...
  const label = entry.is_dir
    ? `folder "${entry.name}" and all its contents`
    : `file "${entry.name}"`
  ...
  try {
    ...
    if (stores) {
      ...
      const updated = openTabs.map((t) =>
        t === entry.path
          ? dst
          : t.startsWith(entry.path + '/')
            ? dst + t.slice(entry.path.length)
            : t,
      )
      stores.openTabs.set(updated)
      // Active file may be the renamed entry itself, or a descendant of a
      // renamed parent directory — both need their path remapped.
      if (openFile === entry.path) {
        stores.openFilePath.set(dst)
      } else if (openFile?.startsWith(entry.path + '/')) {
        stores.openFilePath.set(dst + openFile.slice(entry.path.length))
      }
    }
    await loadTree(projectRoot!)
  } catch (e: any) {
