...
import { handleMenuKeydown, menuFocus } from '$lib/menu-utils'
...
import {
  LSP_CONFIG_RESTART_MS,
  LSP_RESTART_PY_DEBOUNCE_MS,
  PREFS_SAVE_DEBOUNCE_MS,
  SITE_PKG_DEBOUNCE_MS,
  VENV_RECHECK_DEBOUNCE_MS,
} from '$lib/timing'
...
interface Mods {
  ...
  listen: (channel: string, cb: (e: { payload: any }) => void) => Promise<() => void>
  openDialog: () => Promise<any>
  get: <T = any>(store: any) => T
  lspClient: LspClient
...
let toastId = 0
function showToast(message: string, duration = 3000, kind: ToastKind = 'info') {
  const id = toastId++
...
let mods!: Mods // assigned in onMount before any callback fires
...
async function initPrefs(stores: any): Promise<[string | null, Array<() => void>]> {
  ...
  try {
    ...
    if (prefs.active_lsp && VALID_LSP_IDS.includes(prefs.active_lsp))
      stores.activeLsp.set(prefs.active_lsp)
  ...
  const _sessionTrigger = derived([stores.openTabs, stores.openFilePath], (values: unknown[]) => ({
    tabs: values[0] as string[],
    activeFile: values[1] as string | null,
  }))
  unsubs.push(
    _sessionTrigger.subscribe((v: any) => {
...
async function watchConfigFiles(folder: string) {
  ...
  for (const name of ['pyproject.toml', 'ty.toml', 'uv.lock']) {
    const p = `${folder}/${name}`
...
// Restart guard: rapid pythonCmd changes (e.g. user clicking through venvs)
// can interleave stop()/start() calls. Each restart claims a sequence; only
// the latest seq is allowed to complete the LSP start.
let _restartSeq = 0

async function startLsp(root: string, stores: any, seq?: number): Promise<void> {
  ...
  // Don't clear diagnostics here — each LSP restart (e.g. on pythonCmd change)
  // would briefly wipe red squiggles and re-add them, causing visual flicker.
  // The new LSP publishes per-file diagnostics on init, replacing old entries.
  // Project switches and LSP-type switches clear diagnostics explicitly at the
  // call site since their old entries genuinely don't apply anymore.
  try {
    if (seq !== undefined && seq !== _restartSeq) return
...
async function restartLsp(root: string) {
  const seq = ++_restartSeq
  try {
    await mods.lspClient.stop()
  } catch {}
  if (seq !== _restartSeq) return // a newer restart superseded us
  await startLsp(root, mods.stores, seq)
}
...
function findVenvForFile(filePath: string) {
  return venvManager?.findVenvForFile(filePath)
}
function preloadVenvCache(root: string) {
  venvManager?.preloadVenvCache(root)
}
...
async function initLsp(stores: any, lastProject: string | null): Promise<Array<() => void>> {
  ...
  let lspSwitchId = 0
  unsubs.push(
    stores.activeLsp.subscribe(async (_lspId: string) => {
      if (!lspInitialized) {
        lspInitialized = true
        return
      } // skip initial value
      ...
      stores.lspStatus.set('starting')
      // Different LSP, different diagnostic set — clear old entries.
    ...
    try {
      ...
      try {
        ...
        if (lastSession?.tabs?.length) {
          const safeTabs = (lastSession.tabs as string[]).filter((t: string) =>
            isUnder(t, lastProject),
          )
          if (safeTabs.length) {
            stores.openTabs.set(safeTabs)
            const af =
              lastSession.active_file && safeTabs.includes(lastSession.active_file)
                ? lastSession.active_file
                : safeTabs[safeTabs.length - 1]
            stores.openFilePath.set(af)
          }
        }
        // (Terminal restore is handled by Terminal.svelte's onMount, which reads
        // projectSessions[root].terminal_tabs directly. Doing it from here would
        // race with Terminal's own initial addTab.)
      } finally {
  ...
  unsubs.push(
    stores.pythonCmd.subscribe((cmd: string) => {
      ...
      if (_lastToastedVenv) {
        ...
        showToast(`Using ${venvName}${rel}`, 3000, 'success')
      }
      _lastToastedVenv = cmd
    }),
  )

  // When the interpreter changes (walk-up venv discovery, manual picker), restart
  // the LSP so it points at the right site-packages. basedpyright reads pythonPath
  // at startup and caches resolutions; without restart it keeps using the previous
  // interpreter and shows stale "module not found" errors. Skip the initial fire.
  let _lastLspPyCmd: string | null = null
  let _lspRestartOnPyChange: ReturnType<typeof setTimeout> | null = null
  unsubs.push(
    stores.pythonCmd.subscribe((cmd: string) => {
      if (!cmd) return
      if (_lastLspPyCmd === null) {
        _lastLspPyCmd = cmd
        return
      } // skip initial
      if (cmd === _lastLspPyCmd) return
      _lastLspPyCmd = cmd
      if (_lspRestartOnPyChange) clearTimeout(_lspRestartOnPyChange)
      _lspRestartOnPyChange = setTimeout(() => {
        _lspRestartOnPyChange = null
        const root = mods.get(stores.projectRoot) as string | null
        if (root) restartLsp(root)
      }, LSP_RESTART_PY_DEBOUNCE_MS)
    }),
  )
  unsubs.push(() => {
  ...
  unsubs.push(
    stores.openFilePath.subscribe((filePath: string | null) => {
      if (!filePath) return
      findVenvForFile(filePath)
    }),
  )

  // Project fs changed → a venv may have just been created (e.g. `uv venv`).
  // Drop negative cache entries and re-run discovery for the current file.
  let venvRecheckTimer: ReturnType<typeof setTimeout> | null = null
  const unlistenDirChanged = await mods.listen('dir:changed', () => {
    if (venvRecheckTimer) clearTimeout(venvRecheckTimer)
    venvRecheckTimer = setTimeout(() => {
      venvRecheckTimer = null
      venvManager?.invalidateAll()
      const cur = mods.get(stores.openFilePath) as string | null
      if (cur) findVenvForFile(cur)
    }, VENV_RECHECK_DEBOUNCE_MS)
  })
  unsubs.push(() => {
    if (venvRecheckTimer) clearTimeout(venvRecheckTimer)
  ...
  // Whenever pythonCmd points to an interpreter under an env-like prefix, ask
  // main to poll its site-packages mtime. On bump (uv sync / pip install added
  // or removed packages), restart the LSP so basedpyright re-resolves imports.
  // Matches .venv/venv/conda/pyenv layouts — anything with a `bin/python*` or
  // `Scripts/python.exe` tail. Main rejects if site-packages can't be located.
  unsubs.push(
    stores.pythonCmd.subscribe((cmd: string) => {
      const m = cmd?.match(/^(.+?)\/(?:bin\/python[\d.]*|Scripts\/python\.exe)$/)
      if (m) mods.invoke('watch_site_packages', { venvPath: m[1] }).catch(() => {})
      else mods.invoke('unwatch_site_packages').catch(() => {})
    }),
  )

  let sitePkgRestartTimer: ReturnType<typeof setTimeout> | null = null
  const unlistenSitePkg = await mods.listen('site-packages:changed', () => {
    if (sitePkgRestartTimer) clearTimeout(sitePkgRestartTimer)
    sitePkgRestartTimer = setTimeout(() => {
      sitePkgRestartTimer = null
      const root = mods.get(stores.projectRoot) as string | null
      if (root) restartLsp(root)
    }, SITE_PKG_DEBOUNCE_MS)
  })
...
async function initSaveListeners(ipcMod: any, stores: any): Promise<Array<() => void>> {
  ...
  unlistens.push(
    await ipcMod.listen('file:changed-on-disk', (e: any) => {
      ...
      if (lspConfigFiles.has(changedPath)) {
        ...
        lspRestartTimer = setTimeout(() => {
          lspRestartTimer = null
          restartLsp(root)
        }, LSP_CONFIG_RESTART_MS)
        return
      }
      ...
      if (dirty.has(changedPath)) {
        ...
        openDialog({ kind: 'disk-conflict', path: changedPath }).then((action) => {
          if (action === 'save') {
  ...
  unlistens.push(
    await ipcMod.listen('app:before-quit', async () => {
      // Snapshot terminal cwds for the active project so they restore on next launch.
      const root = mods.get(stores.projectRoot) as string | null
      if (root && terminalRef?.getSessionState) {
        try {
          const terminalTabs = await terminalRef.getSessionState()
          const sessions = mods.get(stores.projectSessions) as Record<string, any>
          const existing = sessions[root] ?? { tabs: [], active_file: null }
          mods.stores.projectSessions.set({
            ...sessions,
            [root]: { ...existing, terminal_tabs: terminalTabs },
          })
...
async function initMenuListener(ipcMod: any): Promise<Array<() => void>> {
  const unlistenMenu = await ipcMod.listen('menu:action', (e: any) => {
    ...
    if (action === 'refresh_workspace') doRefreshWorkspace()
  })
  return [unlistenMenu]
}

// Helper actions used by both MenuBar and keyboard shortcuts
function doToggleTree() {
  _showFileTree = !_showFileTree
  mods?.stores?.showFileTree?.set(_showFileTree)
}
function doToggleTerminal() {
  _showTerminal = !_showTerminal
  mods?.stores?.showTerminal?.set(_showTerminal)
}

// Manual workspace refresh — escape hatch for NFS / external changes that the
// recursive watcher may have missed. Reloads the file tree and restarts the LSP
// (clears any cached import resolutions, re-scans workspace).
function doRefreshWorkspace() {
  if (!mods) return
  const root = mods.get(mods.stores.projectRoot) as string | null
  if (!root) return
  mods.invoke('refresh_project').catch(() => {}) // FileTree reloads on dir:changed
  restartLsp(root)
  showToast('Workspace refreshed', 2000, 'info')
}
function doZoomIn() {
  if (!mods) return
...
onMount(async () => {
  try {
    ...
    unsubs.push(
      stores.openTabs.subscribe((v: any) => {
        _openTabs = v
      }),
    )
    unsubs.push(
      stores.showFileTree.subscribe((v: any) => {
        _showFileTree = v
      }),
    )
    unsubs.push(
      stores.showTerminal.subscribe((v: any) => {
        _showTerminal = v
      }),
    )
    unsubs.push(
      stores.showDiagnosticsModal.subscribe((v: any) => {
        diagnosticsOpen = v
      }),
    )
    unsubs.push(
    ...
    unsubs.push(
      stores.dirtyTabs.subscribe((v: Set<string>) => {
        _dirtyTabs = v
      }),
    )
    unsubs.push(
      stores.setupStatus.subscribe((v: any) => {
        if (v) {
          _setupReady = v.ready
          _setupMessage = v.message
        }
      }),
    )
...
async function openFolder() {
  ...
  // Save current project's tab session (editor + terminal) before switching
  ...
  if (currentRoot) {
    const terminalTabs = await (terminalRef?.getSessionState?.() ?? Promise.resolve([]))
    const sessions = mods.get(mods.stores.projectSessions) as Record<string, any>
    mods.stores.projectSessions.set({
      ...sessions,
      [currentRoot]: { tabs: currentTabs, active_file: currentFile, terminal_tabs: terminalTabs },
    })
    await mods.invoke('save_prefs', { prefs: buildPrefsPayload() }).catch(() => {})
  }
  ...
  try {
    ...
    if (session?.tabs?.length) {
      const safeTabs = (session.tabs as string[]).filter((t) => isUnder(t, folder))
      if (safeTabs.length) {
        mods.stores.openTabs.set(safeTabs)
        const af =
          session.active_file && safeTabs.includes(session.active_file)
            ? session.active_file
            : safeTabs[safeTabs.length - 1]
...
function onKeydown(e: KeyboardEvent) {
  ...
  if (mod && e.key === 'w') {
    e.preventDefault()
    if (_openFilePath) closeTab(_openFilePath)
  }
...
function startResizeTerm(e: MouseEvent) {
  resizingTerm = true
  e.preventDefault()
}
...
<div class="toast-container" aria-live="polite" aria-atomic="true">
  {#each toasts as toast (toast.id)}
    ...
    <div class="toast toast-{toast.kind}" role={toast.kind === 'error' || toast.kind === 'warning' ? 'alert' : 'status'}>
      <Icon size={14} class="toast-icon" />
...
<style>
  ...
  .toast {
    ...
    border: 1px solid var(--border); border-left-width: 3px; border-radius: 5px;
    padding: 7px 14px; font-size: 12px;
    box-shadow: 0 3px 10px rgba(0,0,0,0.4);
    animation: toast-in 0.15s ease;
  }
  .toast :global(.toast-icon) { flex-shrink: 0; }
  .toast.toast-info    { border-left-color: #4ea1ff; }
  .toast.toast-info    :global(.toast-icon) { color: #4ea1ff; }
  .toast.toast-success { border-left-color: #5fbf65; }
  .toast.toast-success :global(.toast-icon) { color: #5fbf65; }
  .toast.toast-warning { border-left-color: #e0a64a; }
  .toast.toast-warning :global(.toast-icon) { color: #e0a64a; }
  .toast.toast-error   { border-left-color: #e06c75; }
  .toast.toast-error   :global(.toast-icon) { color: #e06c75; }
  @keyframes toast-in {
